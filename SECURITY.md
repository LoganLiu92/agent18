...
agent18 0.5.0 是集成预览，目前没有承诺长期支持的生产稳定版。仓库提供明确的身份和执行边界、自托管部署指引以及可运行的隔离/恢复验证。
...
当前没有任意 Shell/SQL、网页点击代理、自动部署/代码修复、运行时日志调查、通用文件解析/OCR、多步业务事务或无人审核的自动知识发布。内置诊断与本地恢复演练不能替代真实业务验收和离机灾备。
