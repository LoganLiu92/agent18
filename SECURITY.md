...
agent18 0.8.0 是集成预览，目前没有承诺长期支持的生产稳定版。仓库提供明确的身份和执行边界、自托管部署指引以及可运行的隔离/恢复验证。
...
- 只读 POST 由部署者显式审核，固定方法与平铺 JSON body 字段；READ 声明不证明上游无副作用，写业务仍须走确认与回执协议。请求失败不自动重试。
- 业务事件只能承载受限操作/错误/关联标识，SDK 内存保留最多 5 条、10 分钟。事件是未验证线索，不决定身份或授权；工单提交时冻结上下文并允许省略，持久化后遵守现有 Case 隔离与备份规则。
