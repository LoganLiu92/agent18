...
演练证明当前备份可以在本地 PostgreSQL/角色环境中恢复，并验证基础隔离；它不证明上游 SaaS、模型、DNS、SSO、业务回执和真实用户入口都已经恢复。恢复后的业务回执必须继续向 SaaS 核对。0.5 的恢复核验要求包含 migration 005 的表结构；历史版本的备份应使用对应版本工具恢复，再按迁移流程升级，不能直接假设跨版本兼容。
